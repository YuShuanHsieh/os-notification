...
The linked binary lands at `target/x86_64-pc-windows-gnu/release/notify-agent-windows.exe`. Copy it to a Windows 11 machine and run it (same environment variables as the console head apply). It registers a per-user AppUserModelID, enforces a single running instance per session, and renders native toast notifications including avatar images (downloaded and cached under `%LOCALAPPDATA%\DesktopNotificationAgent\`). It runs as a system tray app (no console window): the custom icon (`notify-agent-windows/assets/app.ico`, embedded via `icon.rc`/`build.rs`) appears in the tray immediately on launch, and right-clicking it shows the running version plus a Close item that shuts the agent down.
...
- Launch the `.exe` — the custom tray icon appears immediately, before the agent has finished connecting to NATS.
