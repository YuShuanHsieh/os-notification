...
- **Windows-username identity (Windows head default, when `NOTIFY_AAD_CLIENT_ID` is unset):** the Windows head derives an identity directly from the signed-in Windows account instead of requiring `NOTIFY_USER_ID` — see `notify-agent-windows/src/windows_identity.rs`. It calls the Win32 `GetUserNameExW` function with `NameSamCompatible` (`secur32.dll`, via the `windows` crate's `Win32_Security_Authentication_Identity` feature) rather than reading the `USERNAME` environment variable, which resolves the SAM-compatible, domain-qualified name (`DOMAIN\username`, or `MACHINENAME\username` on a non-domain-joined machine) — falling back to the plain Win32 `GetUserNameW` (`advapi32.dll`, `Win32_System_WindowsProgramming` feature) if `GetUserNameExW` fails or returns an empty string. The domain qualifier is deliberately kept rather than stripped, so two identically-named accounts in different domains (`CORP\jdoe` vs `CONTOSO\jdoe`) resolve to different identities — `GetUserNameW` alone never returns a domain, so without this the two would silently collide onto the same identity on a domain-joined machine. The resolved name is lowercased, then **sanitized** (every character outside `[a-z0-9_-]` — including `.`, `*`, `>`, whitespace, and the `\` domain separator — mapped to `_`) rather than rejected, since those characters are unsafe to embed directly in the `notify.user.{0}.desktop` subject template (a NATS subject-separator, two wildcard tokens, and a whitespace-tokenized-`SUB`-line hazard, respectively). Because that sanitization alone is lossy (e.g. `"user.name"` and `"user_name"` both sanitize to `user_name`), an 8-hex-character `SHA-256` suffix of the pre-sanitization normalized value is appended, giving `u_{sanitized}_{hash8}` (matching the `u_{oid}` shape of the AAD path) that stays injective even when the readable prefix collides. This is a deliberate, narrowly scoped exception to this codebase's general rule that the OS account name is never used as application identity; see `../context/contracts-and-invariants.md`. The console head and the AAD/device-code path above are both unaffected.
...
The Windows head can optionally export three operational metrics — `agent.events.received`, `agent.events.dropped` (tagged with a `reason` attribute: `queue_full` or `bucket_overflow`), and `agent.render.duration` (per-event agent processing latency, in seconds) — via an OTLP HTTP exporter, configured entirely from the settings file above (`otelEnabled`/`otelExporterEndpoint`/`otelServiceName`) with the usual env-var overrides. These names are dot-namespaced (matching OTel semantic-convention style, e.g. `http.server.request.duration`) rather than pre-suffixed Prometheus style (`..._total`/`..._seconds`), and are identical across all three language implementations of this agent — an OTel-to-Prometheus bridge appends those suffixes itself, so baking them into the instrument name risks a doubled suffix once bridged. It's off by default and stays off unless both `otelEnabled` is true *and* `otelExporterEndpoint` is non-blank.
