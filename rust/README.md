...
- **Windows-username identity (Windows head default, when `NOTIFY_AAD_CLIENT_ID` is unset):** the Windows head derives an identity directly from the signed-in Windows account instead of requiring `NOTIFY_USER_ID` — see `notify-agent-windows/src/windows_identity.rs`. It calls the Win32 `GetUserNameExW` function with `NameSamCompatible` (`secur32.dll`, via the `windows` crate's `Win32_Security_Authentication_Identity` feature) rather than reading the `USERNAME` environment variable, which resolves the SAM-compatible, domain-qualified name (`DOMAIN\username`, or `MACHINENAME\username` on a non-domain-joined machine) — falling back to the plain Win32 `GetUserNameW` (`advapi32.dll`, `Win32_System_WindowsProgramming` feature) if `GetUserNameExW` fails or returns an empty string. Either way, only the portion after the last `\` is used as the identity — the domain/machine qualifier is discarded, since deployments using this Windows head are expected to guarantee account-name uniqueness themselves. The account name is lowercased, then **sanitized** (every character outside `[a-z0-9_-]` — including `.`, `*`, `>`, and whitespace — mapped to `_`) rather than rejected, since those characters are unsafe to embed directly in the `notify.user.{0}.desktop` subject template (a NATS subject-separator, two wildcard tokens, and a whitespace-tokenized-`SUB`-line hazard, respectively), giving the plain sanitized username as the identity (e.g. `jdoe`) — no prefix or suffix. This is a deliberate, narrowly scoped exception to this codebase's general rule that the OS account name is never used as application identity; see `../context/contracts-and-invariants.md`. The console head and the AAD/device-code path above are both unaffected.
